...
from chempropfix.args import HyperoptArgs
from chempropfix.constants import HYPEROPT_LOGGER_NAME
from chempropfix.models import MoleculeModel
from chempropfix.nn_utils import param_count
from chempropfix.train import cross_validate, run_training
from chempropfix.utils import create_logger, makedirs, timeit
from chempropfix.hyperopt_utils import merge_trials, load_trials, save_trials, \
    get_hyperopt_seed, load_manual_trials, build_search_space, save_config
from chempropfix.data.utils import seed_torch
import pdb
...
def hyperopt(args: HyperoptArgs) -> None:
    ...
    * :code:`ffn_num_layers`: The number of feed-forward layers after message passing is selected from {1, 2, 3}
    ...
    # args.hyperopt_checkpoint_dir = os.path.join(args.hyperopt_checkpoint_dir,str(args.seed))
    ...
    # 执行num_iters次cross_validate，在第二次之后缓存数据会变成0，从而导致报错
