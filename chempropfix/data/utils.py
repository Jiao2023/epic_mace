...
from chempropfix.args import PredictArgs, TrainArgs
from chempropfix.features import load_features, load_valid_atom_or_bond_features, is_mol
from chempropfix.rdkit import make_mol

from rdkit.Chem import AllChem
from chempropfix.data.compound_constants import DAY_LIGHT_FG_SMARTS_LIST
from chempropfix.data.compound_tools import *
import pickle
import pdb
import random
import torch
...
def check_data_magnitude(tensor, name=None):
    # 基本统计信息
    stats = {
        "min": tensor.min().item(),
        "max": tensor.max().item(),
        ...
        # 检查接近零的值
        "near_zero": (tensor.abs() < 1e-6).sum().item(),
        # 检查非常小的值
        "very_small": (tensor.abs() < 1e-10).sum().item()
    }
    ...
    # 检查是否有极小值
    if stats["very_small"] > 0:
        print(f"Warning: {name} contains {stats['very_small']} very small values (<1e-10)")
...
# return the name of the smiles column 
...
def filter_invalid_smiles(data: MoleculeDataset) -> MoleculeDataset:
    ...
    for datapoint in tqdm(data):
        ...
        and all(m[0].GetNumHeavyAtoms() + m[1].GetNumHeavyAtoms() > 0 for m in datapoint.mol if isinstance(m, tuple)):
            ...
            error_smiles_list.append(datapoint.smiles)
    return MoleculeDataset(dataset),error_smiles_list
    # else error_smiles_list.append(datapoint.smiles)
...
def get_data(path: str,
    ...
    with open(path) as f:
        ...
        data = MoleculeDataset([
            MoleculeDatapoint(
                ...
                index = index 
            ) for i, (smiles, targets,index) in tqdm(enumerate(zip(all_smiles, all_targets,all_index)),
    ...
    if skip_invalid_smiles:
        ...
        data,error_smiles_list = filter_invalid_smiles(data)
