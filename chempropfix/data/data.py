...
from chempropfix.features import get_features_generator
from chempropfix.features import BatchMolGraph, MolGraph
...
from chempropfix.data.compound_tools import mol_to_geognn_graph_data_MMFF3d
...
def load_cache():
    ...
    if (SMILES_TO_GRAPH == {}) and (get_graph_cache_path() is not None):
        print(f"读取Cache文件 from {get_graph_cache_path()}")
        start_time = time.time()
        with open(get_graph_cache_path(), 'rb+') as file:
            SMILES_TO_GRAPH = pickle.load(file)
        print(f"读取完毕,花费 {time.time() - start_time} s")
    elif SMILES_TO_GRAPH != {}: 
        print(f"SMILES_TO_GRAPH中已加载好数据")
    else:
        print("没有传入cache文件")
        SMILES_TO_GRAPH = {}
...
class MoleculeDatapoint:
    ...
    def __init__(self,
        ...
        if self.features_generator is not None:
            ...
            for fg in self.features_generator:
                ...
                for m, reaction in zip(self.mol, self.is_reaction_list):
                    if not reaction:
                        if m is not None and m.NumHvyAtoms()> 0:
                            self.features.extend(features_generator(m))
                        # for H2
                        elif m is not None and m.NumHvyAtoms() == 0:
                        ...
                        if m[0] is not None and m[1] is not None and m[0].OBMol.NumHvyAtoms() > 0:
                            self.features.extend(features_generator(m[0]))
                        elif m[0] is not None and m[1] is not None and m[0].OBMol.NumHvyAtoms() == 0:
                            self.features.extend(np.zeros(len(features_generator(Chem.MolFromSmiles('C')))))   
        ...
        self.raw_features, self.raw_targets, self.raw_atom_targets, self.raw_bond_targets,self.raw_index = \
            self.features, self.targets, self.atom_targets, self.bond_targets,self.index
    ...
    def mol(self) -> List[Union[Chem.Mol, Tuple[Chem.Mol, Chem.Mol]]]:
        """Gets the corresponding list of openbabel molecules for the corresponding SMILES list."""
    ...
    def reset_features_and_targets(self) -> None:
        ...
        self.features, self.targets, self.atom_targets, self.bond_targets,self.index = \
            self.raw_features, self.raw_targets, self.raw_atom_targets, self.raw_bond_targets,self.index
        self.atom_descriptors, self.atom_features, self.bond_descriptors, self.bond_features = \
            self.raw_atom_descriptors, self.raw_atom_features, self.raw_bond_descriptors, self.raw_bond_features
...
class MoleculeDataset(Dataset):
    ...
    def batch_graph(self) -> List[BatchMolGraph]:
        ...
        if self._batch_graph is None:
            ...
            global SMILES_TO_GRAPH
            mol_graphs = []
            for d in self._data:  # d = MoleculeDataPoint
                mol_graphs_list = []
                for s, m in zip(d.smiles, d.mol): 
                    if s+str(d.index[0]) in SMILES_TO_GRAPH:
                        mol_graph = SMILES_TO_GRAPH[s+str(d.index[0])]
                    else:
                        ...
                        if cache_graph():
                            SMILES_TO_GRAPH[s+str(d.index[0])] = mol_graph
    ...
    def mask(self) -> List[List[bool]]:
        ...
        if self.is_atom_bond_targets:
            ...
            mask = list(zip(*mask)) # 解包再压缩 相当于得到了矩阵的转置
        return mask
...
class MoleculeDataLoader(DataLoader):
    ...
    def gt_targets(self) -> List[List[Optional[bool]]]:
        """
         Returnsbooleans for whether each target is an inequality rather than a value target, associated with each molecule.
...
def make_pybelmols(smiles: List[str], reaction_list: List[bool], keep_h_list: List[bool], add_h_list: List[bool], keep_atom_map_list: List[bool]):
    """
    Builds a list of RDKit molecules (or a list of tuples of molecules if reaction is True) for a list of smiles.
